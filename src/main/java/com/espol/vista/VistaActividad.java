...
public class VistaActividad extends Actividad {
        ...
        for (Actividad act : controlador.getLista()) {
            ...
            if (act.getFechavencimiento().isAfter(LocalDateTime.now())) {
                if (act instanceof ActividadAcademica) {
                    ...
                    System.out.printf(formato, act.getId(), w.getSubtipo(), act.getNombre(), fecha, act.getPrioridad(),
                            avanceFormateado);
                }
                if (act instanceof ActividadPersonal) {

                    System.out.printf(formato, act.getId(), act.getTipo(), act.getNombre(), fecha, act.getPrioridad(),
                            avanceFormateado);
                }
            }
        }
    ...
    public void crearActividadPersonal(ControladorActividad controlador) {
        ...
        // para su posterior conversion a LocalDateTime
        String[] w = t.split("/");
                ...
                if (id == k) {
                    ...
                    if (act instanceof ActividadAcademica) {
                        ...
                        for (SesionEnfoque Sesiones : j) {
                            ...
                            System.out.printf(formato, fecha1, Sesiones.getTecnicaAplicada(),
                                    Sesiones.getDuracionMinutos());
                        }
            ...
            for (Actividad act : controlador.getLista()) {
                ...
                if (k == e) {
                    ...
                    if (act instanceof ActividadAcademica) {
                        ...
                        System.out.println("¿Confirma que el nuevo avance para " + w.getSubtipo().toLowerCase() + " ID "
                                + w.getId() + " es " + t + "%? (S/N): ");
                        String j = o.nextLine();
                        if (j.equals("S")) {
                    ...
                    if (act instanceof ActividadPersonal) {
                        ...
                        o.nextLine();
                        // se confirma el avance
                        System.out.println("¿Confirma que el nuevo avance para la actividad PERSONAL  ID " + w.getId()
                                + " es " + t + "%? (S/N): ");
    ...
    public void eliminarActividad(ControladorActividad controlador) {
        ...
        if (e > 0) {
            ...
            for (Actividad act : controlador.getLista()) {
                int k = act.getId();
                if (k == e) {
                    System.out.println("Usted seleccionó la actividad ID " + act.getId() + ": '" + act.getNombre()
                            + " '" + "(TIPO: " + act.getTipo() + " )");
